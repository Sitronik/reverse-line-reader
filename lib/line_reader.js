(function () {
  ...
  function LineReader(filename, size, separator, encoding, chunkSize) {
    this.size = size || fs.statSync(filename).size;
  ...
  function eachLine(filename, cb, size, separator, encoding, chunkSize) {
    var finalFn, asyncCb = cb.length == 3;
    var reader = new LineReader(filename, size, separator, encoding, chunkSize);
    readLine();
